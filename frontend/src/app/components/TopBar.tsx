...
const subtitles: Record<AppView, string> = {
  dashboard: 'Rebuilt recruitment workspace foundation.',
  jobs: 'Publish and manage agency job advertisements.',
  candidates: 'Agency and candidate onboarding workspace.',
  applications: 'Candidate applications by job.',
  interviews: 'Interview scheduling and panel evaluation.',
  agencies: 'Agency, interviewer, and user administration.',
  settings: 'Platform configuration.',
};

export const TopBar = ({ activeView, onOpenMobileNav }: TopBarProps) => (
  <header className="sticky top-0 z-30 flex min-h-16 items-center gap-3 border-b border-slate-200 bg-white/92 px-3 backdrop-blur-xl sm:px-5">
    ...
    <div className="min-w-0 flex-1 lg:max-w-sm"><p className="truncate text-sm font-bold text-slate-950">{titles[activeView]}</p><p className="hidden truncate text-[11px] text-slate-500 sm:block">{subtitles[activeView]}</p></div>
    ...
    <div className="flex items-center gap-2 rounded-xl px-2 py-1.5"><div className="grid size-9 place-items-center rounded-full bg-slate-900 text-xs font-bold text-white">AD</div><div className="hidden text-left lg:block"><p className="text-xs font-bold text-slate-800">Admin</p><p className="text-[10px] text-slate-500">System administrator</p></div></div>
  </header>
);
