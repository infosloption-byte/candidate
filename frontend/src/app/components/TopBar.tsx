...
export const TopBar = ({ title, subtitle, onSearch, searchValue }: TopBarProps) => {
  ...
  return (
    <header className="sticky top-0 z-30 flex min-h-16 items-center gap-3 border-b border-slate-200 bg-white/92 px-3 backdrop-blur-xl sm:px-5">
      ...
      <label className="hidden min-w-0 flex-1 md:block md:max-w-md">
        <span className="sr-only">Search candidates</span>
        <div className="relative"><Icon name="search" size={16} className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400"/><input id="global-search" value={searchValue} onChange={(event) => onSearch(event.target.value)} placeholder="Search candidates, skills, professions…" className="w-full rounded-xl border border-slate-200 bg-slate-50 py-2.5 pl-9 pr-14 text-sm text-slate-800 outline-none transition placeholder:text-slate-400 focus:border-cyan-300 focus:bg-white"/><kbd className="absolute right-2 top-1/2 -translate-y-1/2 rounded-md border border-slate-200 bg-white px-1.5 py-0.5 text-[10px] font-semibold text-slate-400">/</kbd></div>
      </label>
      ...
      <input className="sr-only" aria-hidden="true" tabIndex={-1} value={title} readOnly />
    </header>
  );
};
