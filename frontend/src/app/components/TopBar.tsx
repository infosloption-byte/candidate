...
export const TopBar = ({ role, activeView, onOpenMobileNav, onRoleChange }: TopBarProps) => (
  <header className="sticky top-0 z-30 flex min-h-16 items-center gap-3 border-b border-slate-200 bg-white/92 px-3 backdrop-blur-xl sm:px-5">
    ...
    <div className="flex items-center gap-2 rounded-xl px-2 py-1.5"><div className="grid size-9 place-items-center rounded-full bg-slate-900 text-xs font-bold text-white">{role === 'INTERVIEWEE' ? 'IN' : role === 'INTERVIEWER' ? 'IR' : role === 'AGENCY' ? 'AG' : 'AD'}</div><div className="hidden text-left lg:block"><p className="text-xs font-bold text-slate-800">{roleLabels[role]}</p><p className="text-[10px] text-slate-500">Development session</p></div></div>
  </header>
);
