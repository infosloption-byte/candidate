...
export const AuthPasswordDialog = () => {
  ...
  if (state.authView !== 'change-password' || !state.authenticated) return null;
  ...
  return <div className="fixed inset-0 z-[70] grid place-items-center bg-slate-950/50 p-4" role="dialog" aria-modal="true" aria-labelledby="change-password-title">
    <div className="w-full max-w-md rounded-3xl bg-white p-6 shadow-2xl">
      <div className="flex items-start justify-between gap-4">
        ...
        <button type="button" onClick={() => dispatch({ type: 'COMPLETE_CHANGE_PASSWORD', notice: '' })} title="Close change password" className="rounded-lg px-2 py-1 text-slate-400">×</button>
      </div>
      {error && <p role="alert" className="mt-4 rounded-xl bg-rose-50 p-3 text-xs font-semibold text-rose-700">{error}</p>}
      <label className="field-label mt-5">Current password<input value={currentPassword} onChange={(event) => setCurrentPassword(event.target.value)} type="password" className="field-input" /></label>
      <label className="field-label mt-4">New password<input value={nextPassword} onChange={(event) => setNextPassword(event.target.value)} type="password" className="field-input" /></label>
      <label className="field-label mt-4">Confirm new password<input value={confirmPassword} onChange={(event) => setConfirmPassword(event.target.value)} type="password" className="field-input" /></label>
      <div className="mt-5 flex justify-end gap-2"><button type="button" onClick={() => dispatch({ type: 'COMPLETE_CHANGE_PASSWORD', notice: '' })} title="Cancel password change" className="rounded-xl border border-slate-200 px-4 py-2.5 text-xs font-bold text-slate-600">Cancel</button><button type="button" onClick={submit} title="Save new password" className="rounded-xl bg-slate-950 px-4 py-2.5 text-xs font-bold text-white">Change password</button></div>
    </div>
  </div>;
};
