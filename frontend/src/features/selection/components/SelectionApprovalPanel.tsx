...
export const SelectionApprovalPanel = ({ job, selectedCount, reserveCount, approvalStatus, approvalNote, approvalReady, onSetApproval }: SelectionApprovalPanelProps) => {
  ...
  const submitForApproval = () => onSetApproval('pending', note.trim());
  const approve = () => onSetApproval('approved', note.trim() || 'Approved by management.');
  const returnToRecruiter = () => {
    if (!note.trim()) return;
    onSetApproval('returned', note.trim());
  };
  ...
  return (
    <section className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:p-5" aria-labelledby="approval-title">
      ...
      <label className="mt-4 block"><span className="field-label">Approval note</span><textarea value={note} onChange={(event) => setNote(event.target.value)} rows={3} className="field-input min-h-20 resize-y bg-white" placeholder="Message for management or approval context…"/></label>
      ...
      <div className="mt-4 flex flex-wrap gap-2">
        {approvalStatus === 'draft' || approvalStatus === 'returned' ? <button type="button" onClick={submitForApproval} disabled={!approvalReady} className="rounded-xl bg-slate-900 px-3.5 py-2.5 text-[10px] font-black text-white disabled:cursor-not-allowed disabled:opacity-40">Submit for approval</button> : null}
        {approvalStatus === 'pending' ? <><button type="button" onClick={approve} className="rounded-xl bg-emerald-600 px-3.5 py-2.5 text-[10px] font-black text-white hover:bg-emerald-700">Approve shortlist</button><button type="button" onClick={returnToRecruiter} disabled={!note.trim()} className="rounded-xl border border-rose-200 bg-rose-50 px-3.5 py-2.5 text-[10px] font-black text-rose-700 disabled:opacity-40">Return for review</button></> : null}
