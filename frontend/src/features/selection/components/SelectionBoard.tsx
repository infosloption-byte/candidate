...
export const SelectionBoard = ({ job, jobs, tabRows, selectedRow, history, activeTab, selectedCount, reserveCount, recommendedCount, remaining, approvalStatus, approvalReady, approvalNote, bulkSelectedIds, bulkAllVisibleSelected, bulkReason, bulkNote, bulkTargetJobId, bulkError, onChangeJob, onTabChange, onSelectCandidate, onToggleBulkCandidate, onToggleAllBulk, onClearBulk, onBulkReasonChange, onBulkNoteChange, onBulkTargetJobChange, onBulkSelect, onBulkReserve, onBulkReject, onBulkReassign, onDecision, onApproval }: SelectionBoardProps) => (
  <div className="flex min-h-full flex-col bg-slate-50">
    ...
    <div className="grid min-h-0 flex-1 xl:grid-cols-[420px_minmax(0,1fr)]">
      ...
      <section className="min-w-0 overflow-y-auto p-3 sm:p-4 xl:p-5"><div className="mx-auto max-w-3xl space-y-4"><SelectionEvidencePanel row={selectedRow}/><SelectionDecisionPanel row={selectedRow} job={job} selectedCount={selectedCount} onSubmit={onDecision}/>{selectedCount > 0 && <SelectionApprovalPanel job={job} selectedCount={selectedCount} reserveCount={reserveCount} approvalStatus={approvalStatus} approvalNote={approvalNote} approvalReady={approvalReady} onSetApproval={onApproval}/>}<SelectionHistoryPanel entries={history}/></div></section>
    </div>
  </div>
);
