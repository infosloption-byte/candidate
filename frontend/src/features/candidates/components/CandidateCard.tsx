...
interface CandidateCardProps { candidate: Candidate; selected: boolean; onSelect: (candidateId: string) => void; }
...
export const CandidateCard = ({ candidate, selected, onSelect }: CandidateCardProps) => {
  ...
  return (
    <button type="button" aria-pressed={selected} onClick={() => onSelect(candidate.id)} className={`group flex w-full items-start gap-3 border-b border-slate-100 px-3 py-3.5 text-left transition sm:px-4 ${selected ? 'bg-cyan-50/70' : 'bg-white hover:bg-slate-50'}`}>
      <div className="grid size-10 shrink-0 place-items-center rounded-xl bg-slate-900 text-xs font-bold text-white">{initials}</div>
