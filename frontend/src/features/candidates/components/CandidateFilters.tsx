...
export const CandidateFilters = ({ search, status, profession, professions, resultCount, onSearchChange, onStatusChange, onProfessionChange }: CandidateFiltersProps) => {
  ...
  return (
    <div className="border-b border-slate-200 bg-white p-3 sm:p-4">
      ...
      <div className="mt-3 flex items-center justify-between text-[11px] text-slate-400"><span className="inline-flex items-center gap-1.5"><Icon name="sliders" size={13} /> Smart filters</span><span>{resultCount} matching</span></div>
    </div>
  );
};
