      ...
      <div className="grid min-h-0 flex-1 xl:grid-cols-[380px_minmax(0,1fr)]">
        <section className={`${mobileDetailOpen ? 'hidden xl:flex' : 'flex'} min-h-0 flex-col border-b border-slate-200 bg-white xl:border-b-0 xl:border-r`} aria-label="Candidate directory">
          <CandidateFilters
            ...
            smartFilterCount={smartFilterCount}
            smartFilters={state.smartFilters}
            smartFiltersOpen={smartFiltersOpen}
            onSearchChange={actions.setSearch}
            onStatusChange={actions.setStatus}
            onProfessionChange={actions.setProfession}
            onOpenSmartFilters={() => setSmartFiltersOpen((current) => !current)}
          />
          ...
          <div className="scrollbar-thin min-h-0 flex-1 overflow-y-auto">
            {visibleCandidates.length === 0 ? <div className="grid min-h-64 place-items-center p-6 text-center"><div><div className="mx-auto grid size-12 place-items-center rounded-2xl bg-slate-100 text-slate-400"><Icon name="search" size={21}/></div><h2 className="mt-4 text-sm font-bold text-slate-800">No candidates found</h2><p className="mt-1 text-xs leading-5 text-slate-500">No candidate matches all of the current criteria.</p><div className="mt-4 flex flex-wrap justify-center gap-2"><button type="button" onClick={() => { actions.clearSmartFilters(); actions.setStatus('all'); actions.setProfession('all'); actions.setSearch(''); }} className="rounded-xl border border-slate-200 bg-white px-3.5 py-2.5 text-xs font-bold text-slate-700">Clear filters</button><button type="button" onClick={actions.openAddCandidate} className="rounded-xl bg-slate-900 px-3.5 py-2.5 text-xs font-bold text-white">Add candidate</button></div></div></div> : visibleCandidates.map((candidate) => <CandidateCard key={candidate.id} candidate={candidate} selected={candidate.id === selectedCandidate?.id} compareSelected={state.compareCandidateIds.includes(candidate.id)} compareDisabled={state.compareCandidateIds.length >= 4} onSelect={(id) => { actions.selectCandidate(id); setMobileDetailOpen(true); }} onToggleCompare={actions.toggleCompareCandidate}/>) }
          </div>
        </section>
