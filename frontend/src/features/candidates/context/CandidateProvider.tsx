...
const candidateReducer = (state: CandidateState, action: CandidateAction): CandidateState => {
  switch (action.type) {
    ...
    case 'UPDATE_DOCUMENTS': return { ...state, candidates: state.candidates.map((candidate) => candidate.id === action.candidateId ? { ...candidate, documents: action.documents, journey: [action.journeyEvent, ...candidate.journey] } : candidate) };
