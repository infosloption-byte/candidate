...
import { findDuplicateMatchesForDraft } from '../services/candidateMatching';
import type { Candidate, CandidateDraft, CandidateDuplicateMatch, CandidateSource, EnglishLevel, Availability } from '../types/candidate';
...
export const useCandidateForm = (existingCandidates: Candidate[], onCreate: (candidate: Candidate) => void | Promise<void>, onClose: () => void) => {
  ...
  const duplicateMatches = useMemo<CandidateDuplicateMatch[]>(() => findDuplicateMatchesForDraft(existingCandidates, draft), [existingCandidates, draft]);
  const hasHighConfidenceDuplicate = duplicateMatches.some((match) => match.confidence === 'high');

  const updateField = <K extends keyof CandidateDraft>(field: K, value: CandidateDraft[K]) => {
  ...
  const submit = async () => {
    ...
    const candidate: Candidate = {
      id: makeId(),
      reference: `CA-${Math.floor(1000 + Math.random() * 8999)}`,
      ...
      status: 'new',
      fitScore: 0,
      documents: { passport: draft.passportNumber.trim() ? 'needs-review' : 'missing', cv: 'missing', tradeCertificate: 'missing' },
  ...
  const reset = () => { setDraft(emptyDraft); setStep(1); setError(null); setDuplicateAcknowledged(false); setSaved(false); };

  return useMemo(() => ({ draft, step, error, saved, duplicateMatches, hasHighConfidenceDuplicate, duplicateAcknowledged, setDuplicateAcknowledged, updateField, next, back, submit, setEnglishLevel, setAvailability, setSource, reset }), [draft, step, error, saved, duplicateMatches, hasHighConfidenceDuplicate, duplicateAcknowledged]);
};
