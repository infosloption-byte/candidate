...
const slotConflicts = (slot: { date: string; time: string; interviewerId: string; durationMinutes: number; location: string }, interviews: Interview[], sharedLocation: boolean): string[] => {
...
export const planBulkInterviewSchedule = (candidates: Candidate[], interviewers: Interviewer[], existingInterviews: Interview[], config: BulkInterviewScheduleConfig): BulkInterviewSchedulePlan => {
  ...
  const unscheduled: BulkInterviewSchedulePlan['unscheduled'] = requestedCandidates.filter((candidate) => candidateBusy.has(candidate.id)).map((candidate) => ({ candidateId: candidate.id, candidateName: candidate.name, reason: 'Already has an active interview.' }));
  if (activeInterviewers.length === 0 || dayEnd <= dayStart || step <= 0 || endDate < startDate) return { slots: [], unscheduled: [...unscheduled, ...availableCandidates.map((candidate) => ({ candidateId: candidate.id, candidateName: candidate.name, reason: 'The schedule configuration does not contain a usable interviewer pool or time window.' }))], capacity: 0, requested: requestedCandidates.length };
  const dates: Date[] = [];
  for (let cursor = new Date(startDate); cursor <= endDate; cursor.setDate(cursor.getDate() + 1)) { const day = cursor.getDay(); if (config.includeWeekends || (day !== 0 && day !== 6)) dates.push(new Date(cursor)); }
  ...
  for (const date of dates) {
    ...
    for (let time = dayStart; time + config.durationMinutes <= dayEnd && candidateIndex < availableCandidates.length; time += step) {
      const interviewerOrder = [...activeInterviewers].sort((left, right) => { const leftLoad = reserved.filter((slot) => slot.date === iso && slot.interviewerId === left.id).length; const rightLoad = reserved.filter((slot) => slot.date === iso && slot.interviewerId === right.id).length; return leftLoad - rightLoad || left.id.localeCompare(right.id); });
      for (const interviewer of interviewerOrder) {
        ...
        if (matchingPool.length > 0 && !matchingPool.some((person) => person.id === interviewer.id)) continue;
        ...
        if (slotConflicts(slot, existingInterviews, config.sharedLocation).length > 0) continue;
        const reservedConflict = reserved.some((item) => item.date === slot.date && overlaps(toMinutes(slot.time), slot.durationMinutes, toMinutes(item.time), item.durationMinutes) && (item.interviewerId === slot.interviewerId || (config.sharedLocation && item.location && slot.location && item.location.toLowerCase() === slot.location.toLowerCase())));
  ...
  for (const candidate of availableCandidates) if (!scheduledIds.has(candidate.id)) unscheduled.push({ candidateId: candidate.id, candidateName: candidate.name, reason: 'No free interviewer slot remained in the selected window.' });
