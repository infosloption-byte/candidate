      ...
      <header className="border-b border-slate-200 bg-white px-4 py-4 sm:px-6">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <div className="inline-flex rounded-xl border border-slate-200 bg-white"><button type="button" onClick={() => onMove(-1)} aria-label={view === 'week' ? 'Previous week' : 'Previous day'} className="grid size-9 place-items-center text-slate-500 hover:bg-slate-50"><Icon name="chevron-left" size={16}/></button><button type="button" onClick={() => onMove(1)} aria-label={view === 'week' ? 'Next week' : 'Next day'} className="grid size-9 place-items-center border-l border-slate-200 text-slate-500 hover:bg-slate-50"><Icon name="chevron-right" size={16}/></button></div>
            <button type="button" onClick={onSchedule} className="inline-flex items-center gap-2 rounded-xl bg-slate-900 px-3.5 py-2.5 text-[11px] font-bold text-white shadow-sm hover:bg-slate-800"><Icon name="plus" size={14}/> Schedule</button>
          </div>
        </div>
        <label className="mt-3 inline-flex items-center gap-2 rounded-xl border border-slate-200 bg-slate-50 px-3 py-2"><span className="text-[10px] font-bold text-slate-400">Jump to</span><input aria-label="Jump to calendar date" type="date" value={firstDay?.isoDate ?? ''} onChange={(event) => event.target.value && onDateChange(event.target.value)} className="bg-transparent text-xs font-bold text-slate-700 outline-none"/></label>
      </header>
        ...
        <div className="hidden min-w-[920px] md:block">
          <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
            <div className="grid" style={{ gridTemplateColumns: `76px repeat(${days.length}, minmax(0, 1fr))` }}>
              <div className="border-b border-r border-slate-200 bg-slate-50 p-3 text-[9px] font-bold uppercase tracking-wider text-slate-400">Time</div>
              {days.map((day) => <div key={day.isoDate} className={`border-b border-slate-200 bg-slate-50 p-3 ${day.isToday ? 'bg-cyan-50/60' : ''}`}><p className="text-[10px] font-black text-slate-400">{day.shortLabel.toUpperCase()}</p><p className={`mt-1 text-sm font-black ${day.isToday ? 'text-cyan-700' : 'text-slate-800'}`}>{day.date.getDate()}</p></div>)}
            </div>
