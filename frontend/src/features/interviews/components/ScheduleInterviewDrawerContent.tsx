      ...
      <aside ref={drawerRef} tabIndex={-1} className="absolute inset-y-0 right-0 flex w-full max-w-2xl flex-col bg-white shadow-2xl sm:rounded-l-3xl">
        ...
        <form id="schedule-interview-form" onSubmit={(event) => { event.preventDefault(); form.submit(); }} className="scrollbar-thin min-h-0 flex-1 overflow-y-auto px-4 py-5 sm:px-6">
          ...
          <section className="mt-6" aria-labelledby="schedule-time-title">
            ...
            <div className="mt-3 grid gap-4 sm:grid-cols-3"><label><span className="field-label">Date <span className="text-rose-500">*</span></span><input required aria-required="true" title="Choose the interview date" type="date" value={form.draft.date} onChange={(event) => form.updateField('date', event.target.value)} className="field-input"/></label><label><span className="field-label">Start time <span className="text-rose-500">*</span></span><input required aria-required="true" title="Choose when the interview starts" type="time" value={form.draft.time} onChange={(event) => form.updateField('time', event.target.value)} className="field-input"/></label><label><span className="field-label">Duration <span className="text-rose-500">*</span></span><select required aria-required="true" title="Choose the expected interview duration" value={form.draft.durationMinutes} onChange={(event) => form.updateField('durationMinutes', event.target.value)} className="field-input"><option value="30">30 min</option><option value="45">45 min</option><option value="60">60 min</option><option value="90">90 min</option></select></label></div>
            <label className="mt-4 block"><span className="field-label">Location or meeting link <span className="text-rose-500">*</span></span><input required aria-required="true" title="Enter the interview room, yard, workstation, or online meeting link" value={form.draft.location} onChange={(event) => form.updateField('location', event.target.value)} className="field-input" placeholder="Interview room, practical yard, workstation, or online link"/></label>
          </section>

          <section className="mt-6" aria-labelledby="schedule-interviewer-title">
            <div className="flex items-end justify-between gap-3"><div><h3 id="schedule-interviewer-title" className="text-sm font-black text-slate-900">Interviewers <span className="text-rose-500">*</span></h3><p className="mt-1 text-[11px] text-slate-500">Assign one or more active interviewers. Their specialties and current appointments are checked before scheduling.</p></div><span className="text-[10px] font-bold text-slate-400">{form.draft.interviewerIds.length} selected</span></div>
            <div className="mt-3 space-y-2">{interviewers.filter((person) => person.active).map((person) => { const selected = form.draft.interviewerIds.includes(person.id); return <button key={person.id} type="button" aria-pressed={selected} title={`${selected ? 'Remove' : 'Assign'} ${person.name} as an interviewer. Specialties: ${person.specialties.join(', ')}`} onClick={() => form.toggleInterviewer(person.id)} className={`flex w-full items-start gap-3 rounded-2xl border p-3 text-left ${selected ? 'border-cyan-300 bg-cyan-50' : 'border-slate-200 bg-white hover:bg-slate-50'}`}><span className={`mt-0.5 grid size-5 shrink-0 place-items-center rounded-full border text-[10px] font-black ${selected ? 'border-cyan-600 bg-cyan-600 text-white' : 'border-slate-300 text-transparent'}`}>✓</span><span className="min-w-0 flex-1"><span className="block text-xs font-black text-slate-800">{person.name}</span><span className="mt-0.5 block text-[10px] text-slate-500">{person.role}</span><span className="mt-1 block truncate text-[10px] text-slate-400">{person.specialties.join(' · ')}</span></span></button>; })}</div>
          </section>

          <section className="mt-6" aria-labelledby="schedule-check-title">
