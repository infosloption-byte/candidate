...
export const InterviewScorecard = ({ interview, completedCriteria, totalCriteria, totalScore, onScore, onCriterionNote }: InterviewScorecardProps) => (
        ...
        <article key={criterion.id} className="rounded-2xl border border-slate-100 bg-slate-50/60 p-3.5 sm:p-4">
          <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
            ...
            <div className="flex gap-1.5" role="radiogroup" aria-label={`${criterion.label} score`}>
              {scoreValues.map((value) => <button key={value} type="button" role="radio" aria-checked={criterion.score === value} aria-label={`${criterion.label}: ${value} out of 5`} onClick={() => onScore(criterion.id, value)} className={`grid size-9 place-items-center rounded-xl border text-xs font-black transition ${criterion.score === value ? 'border-cyan-600 bg-cyan-600 text-white' : 'border-slate-200 bg-white text-slate-500 hover:border-cyan-300 hover:text-cyan-700'}`}>{value}</button>)}
              <button type="button" aria-label={`Clear ${criterion.label} score`} disabled={criterion.score === null} onClick={() => onScore(criterion.id, null)} className="ml-1 grid size-9 place-items-center rounded-xl border border-slate-200 bg-white text-slate-400 disabled:opacity-40"><Icon name="x" size={14}/></button>
            </div>
          </div>
          <label className="mt-3 block"><span className="field-label">Interviewer note <span className="font-normal text-slate-400">(optional)</span></span><input value={criterion.note} onChange={(event) => onCriterionNote(criterion.id, event.target.value)} className="field-input bg-white" placeholder="What did you observe?"/></label>
        </article>
      ))}
    </div>
  </section>
