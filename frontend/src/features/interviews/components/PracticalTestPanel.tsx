...
interface PracticalTestPanelProps {
  interview: Interview;
  onResult: (itemId: string, result: PracticalResult, note: string) => void;
}

const resultOptions: Array<{ value: PracticalResult; label: string; className: string }> = [
...
export const PracticalTestPanel = ({ interview, onResult }: PracticalTestPanelProps) => (
  <section className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:p-5" aria-labelledby="practical-test-title">
    <div className="flex items-start justify-between gap-3">
      <div><div className="flex items-center gap-2"><div className="grid size-9 place-items-center rounded-xl bg-amber-50 text-amber-700"><Icon name="briefcase" size={16}/></div><div><h2 id="practical-test-title" className="text-sm font-black text-slate-900">Practical test</h2><p className="mt-0.5 text-[11px] text-slate-500">Record what the candidate demonstrated on the task.</p></div></div></div>
      <span className="rounded-full bg-slate-100 px-2 py-1 text-[9px] font-bold text-slate-500">Required tasks must be decided</span>
    </div>
      ...
      {interview.practicalTest.map((item) => (
        <article key={item.id} className="rounded-2xl border border-slate-100 bg-slate-50/60 p-3.5 sm:p-4">
          <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
            ...
            <div className="flex flex-wrap gap-1.5">{resultOptions.map((option) => <button key={option.value} type="button" aria-pressed={item.result === option.value} onClick={() => onResult(item.id, option.value, item.note)} className={`rounded-xl border px-3 py-2 text-[10px] font-bold transition ${item.result === option.value ? option.className : 'border-slate-200 bg-white text-slate-500 hover:bg-slate-50'}`}>{option.label}</button>)}</div>
          </div>
          <label className="mt-3 block"><span className="field-label">Observation <span className="font-normal text-slate-400">(recommended)</span></span><input value={item.note} onChange={(event) => onResult(item.id, item.result === 'not-started' ? 'pending' : item.result, event.target.value)} className="field-input bg-white" placeholder="e.g. Clean alignment, safe tool handling, slow execution…"/></label>
        </article>
      ))}
    </div>
  </section>
