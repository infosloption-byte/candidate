    ...
    <section className="flex min-h-0 flex-col border-b border-slate-200 bg-white xl:border-b-0 xl:border-r" aria-label="Interview queue">
      <header className="border-b border-slate-200 p-4 sm:p-5">
        <div className="flex items-start justify-between gap-3">
          ...
          <div className="flex shrink-0 flex-wrap justify-end gap-2">{onBulkSchedule && <button type="button" onClick={onBulkSchedule} className="inline-flex items-center gap-1.5 rounded-xl border border-cyan-200 bg-cyan-50 px-3 py-2.5 text-xs font-bold text-cyan-700 hover:bg-cyan-100"><Icon name="calendar" size={14}/>Batch</button>}{onSchedule && <button type="button" onClick={onSchedule} className="inline-flex items-center gap-1.5 rounded-xl bg-slate-900 px-3 py-2.5 text-xs font-bold text-white hover:bg-slate-800"><Icon name="plus" size={14}/>Schedule</button>}</div>
        </div>
