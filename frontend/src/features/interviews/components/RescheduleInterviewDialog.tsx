...
export const RescheduleInterviewDialog = ({ open, interview, interviewers, draft, error, alternatives, onClose, onDateChange, onTimeChange, onToggleInterviewer, onReasonChange, onUseAlternative, onSave }: RescheduleInterviewDialogProps) => {
  ...
  return (
    <div className="fixed inset-0 z-[60]" role="dialog" aria-modal="true" aria-labelledby="reschedule-interview-title">
      <button type="button" className="absolute inset-0 bg-slate-950/50 backdrop-blur-sm" aria-label="Close reschedule interview dialog" title="Close without saving this schedule change" onClick={onClose}/>
      <aside ref={dialogRef} tabIndex={-1} className="absolute inset-y-0 right-0 flex w-full max-w-xl flex-col overflow-hidden bg-white shadow-2xl sm:rounded-l-3xl">
        ...
        <div className="scrollbar-thin min-h-0 flex-1 overflow-y-auto px-4 py-5 sm:px-6">
          ...
          <section className="mt-4 rounded-2xl border border-slate-200 bg-slate-50 p-4" aria-labelledby="reschedule-current-title" title="Current appointment details"><h3 id="reschedule-current-title" className="text-[10px] font-black uppercase tracking-wider text-slate-400">Current appointment</h3><p className="mt-2 text-sm font-black text-slate-900">{interview.date} at {interview.time}</p><p className="mt-1 text-[11px] text-slate-500">{interview.interviewers.map((person) => person.name).join(', ') || 'No interviewer'} · {interview.location}</p></section>

          <section className="mt-5" aria-labelledby="reschedule-target-title"><div><h3 id="reschedule-target-title" className="text-sm font-black text-slate-900">New appointment</h3><p className="mt-1 text-[11px] text-slate-500">Choose a new date, start time, and interviewer panel. Every change is checked before saving.</p></div><div className="mt-3 grid gap-4 sm:grid-cols-2"><label><span className="field-label">Date <span className="text-rose-500">*</span></span><input required aria-required="true" title="Choose the new interview date" type="date" value={draft.date} onChange={(event) => onDateChange(event.target.value)} className="field-input"/></label><label><span className="field-label">Start time <span className="text-rose-500">*</span></span><input required aria-required="true" title="Choose the new interview start time" type="time" value={draft.time} onChange={(event) => onTimeChange(event.target.value)} className="field-input"/></label></div></section>

          <fieldset className="mt-5"><legend className="text-sm font-black text-slate-900">Interviewers <span className="text-rose-500">*</span></legend><p className="mt-1 text-[11px] text-slate-500">Choose the full panel. Inactive interviewers cannot be selected.</p><div className="mt-3 space-y-2">{interviewers.map((person) => { const selected = draft.interviewerIds.includes(person.id); return <label key={person.id} title={`${person.active ? (selected ? 'Remove' : 'Assign') : 'Inactive interviewer'} ${person.name}. Specialties: ${person.specialties.join(', ')}`} className={`flex items-start gap-3 rounded-2xl border p-3 ${selected ? 'border-cyan-300 bg-cyan-50' : 'border-slate-200 bg-white'} ${person.active ? 'cursor-pointer' : 'cursor-not-allowed opacity-55'}`}><input type="checkbox" checked={selected} disabled={!person.active} onChange={() => onToggleInterviewer(person.id)} className="mt-1 size-4 rounded border-slate-300 text-cyan-600 focus:ring-cyan-300"/><span className="min-w-0 flex-1"><span className="block text-xs font-black text-slate-800">{person.name}</span><span className="mt-0.5 block text-[10px] text-slate-500">{person.role}</span><span className="mt-1 block truncate text-[10px] text-slate-400">{person.specialties.join(' · ')}</span></span>{!person.active && <span className="text-[9px] font-bold text-slate-400">Inactive</span>}</label>; })}</div></fieldset>

          <label className="mt-5 block"><span className="field-label">Reason <span className="text-slate-400">(optional)</span></span><textarea title="Optional explanation recorded with the schedule history" rows={3} value={draft.reason} onChange={(event) => onReasonChange(event.target.value)} className="field-input min-h-20 resize-y" placeholder="Why was this appointment moved?"/></label>

          {alternatives.length > 0 && <section className="mt-5 rounded-2xl border border-amber-200 bg-amber-50 p-4" aria-labelledby="reschedule-alternatives-title"><div className="flex gap-2"><Icon name="alert" size={15} className="mt-0.5 text-amber-700"/><div><h3 id="reschedule-alternatives-title" className="text-xs font-black text-amber-900">Conflict-free alternatives</h3><p className="mt-1 text-[10px] leading-5 text-amber-800/80">Pick an available nearby slot and then save the reschedule.</p><div className="mt-3 grid gap-2">{alternatives.map((alternative) => <button key={`${alternative.date}-${alternative.time}-${alternative.interviewerIds.join('-')}`} type="button" onClick={() => onUseAlternative(alternative)} title={`Use ${alternative.label}`} className="rounded-xl border border-amber-200 bg-white px-3 py-2.5 text-left text-[10px] font-bold text-amber-900 hover:bg-amber-100">{alternative.label}</button>)}</div></div></div></section>}
        </div>

        <footer className="safe-bottom flex items-center justify-end gap-2 border-t border-slate-200 bg-white px-4 py-3 sm:px-6"><button type="button" onClick={onClose} title="Discard the reschedule changes" className="rounded-xl px-4 py-2.5 text-xs font-bold text-slate-600 hover:bg-slate-100">Cancel</button><button type="button" onClick={onSave} title="Validate and save the new interview date, time, and interviewer panel" className="inline-flex items-center gap-2 rounded-xl bg-slate-900 px-4 py-2.5 text-xs font-bold text-white shadow-sm hover:bg-slate-800">Save reschedule<Icon name="check" size={15}/></button></footer>
      </aside>
    </div>
  );
};
