...
export const JobsPage = () => {
  ...
  return <section className="mx-auto max-w-7xl p-4 sm:p-6">
    <header className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm sm:p-7">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between"><div><p className="text-[10px] font-bold uppercase tracking-[0.2em] text-cyan-700">Manpower planning</p><h1 className="mt-2 text-2xl font-black tracking-tight text-slate-950 sm:text-4xl">Manage jobs, vacancies and recruitment demand.</h1><p className="mt-2 max-w-2xl text-sm leading-6 text-slate-500">Define the role requirements that feed selection, allocation and interview planning.</p></div><button type="button" onClick={actions.openCreate} title="Create a new job requirement" className="inline-flex items-center gap-2 rounded-xl bg-slate-950 px-4 py-2.5 text-xs font-bold text-white"><Icon name="plus" size={16}/>Create job</button></div>
    ...
    <div className="mt-4 grid gap-4 md:grid-cols-2 xl:grid-cols-3">
      {visibleJobs.length === 0 ? <div className="md:col-span-2 xl:col-span-3 rounded-2xl border border-dashed border-slate-300 bg-white p-10 text-center"><Icon name="briefcase" size={28} className="mx-auto text-slate-400"/><h2 className="mt-3 text-sm font-black text-slate-900">No jobs match the current filters</h2><p className="mt-1 text-xs text-slate-500">Create a new requirement or clear the search.</p></div> : visibleJobs.map((job) => {
        ...
        return <article key={job.id} className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
          ...
          <div className="mt-4 flex flex-wrap gap-2 border-t border-slate-100 pt-3"><button type="button" onClick={()=>actions.openEdit(job)} title={'Edit ' + job.title} className="rounded-lg border border-slate-200 px-3 py-2 text-[11px] font-bold text-slate-700 hover:bg-slate-50">Edit</button><button type="button" onClick={()=>dispatch({type:'SET_VIEW',view:'selection'})} title="Open candidate selection for this job" className="rounded-lg bg-slate-900 px-3 py-2 text-[11px] font-bold text-white">Selection</button><button type="button" onClick={()=>dispatch({type:'SET_VIEW',view:'allocation'})} title="Open cross-job candidate allocation" className="rounded-lg border border-cyan-200 bg-cyan-50 px-3 py-2 text-[11px] font-bold text-cyan-800">Allocate</button>{job.status !== 'closed' && <button type="button" onClick={()=>actions.closeJob(job.id)} title={'Close ' + job.title} className="rounded-lg border border-rose-200 px-3 py-2 text-[11px] font-bold text-rose-700">Close</button>}</div>
        </article>;
      })}
    </div>
    ...
    {editorOpen && <div className="fixed inset-0 z-50 flex items-end bg-slate-950/45 p-2 sm:items-center sm:justify-center sm:p-6"><div ref={editorRef} tabIndex={-1} className="max-h-[92dvh] w-full max-w-2xl overflow-y-auto rounded-3xl bg-white p-5 shadow-2xl sm:p-7" role="dialog" aria-modal="true" aria-labelledby="job-editor-title"><div className="flex items-start justify-between gap-4"><div><p className="text-[10px] font-bold uppercase tracking-wider text-cyan-700">Job requirement</p><h2 id="job-editor-title" className="mt-1 text-xl font-black text-slate-950">{editingId ? 'Edit job' : 'Create job'}</h2></div><button type="button" onClick={actions.closeEditor} title="Close job editor" aria-label="Close job editor" className="grid size-9 place-items-center rounded-xl border border-slate-200"><Icon name="x" size={17}/></button></div>
      ...
      <div className="mt-6 flex justify-end gap-2 border-t border-slate-100 pt-4"><button type="button" onClick={actions.closeEditor} title="Cancel job editing" className="rounded-xl px-4 py-2.5 text-xs font-bold text-slate-600 hover:bg-slate-100">Cancel</button><button type="button" onClick={actions.save} title={editingId ? 'Save job changes' : 'Create this job requirement'} className="rounded-xl bg-slate-950 px-4 py-2.5 text-xs font-bold text-white">{editingId ? 'Save changes' : 'Create job'}</button></div>
    </div></div>}
  </section>;
};
